...
type Screen = { kind: 'join' } | { kind: 'lobby'; session: PlayerSession; snapshot?: RoomSnapshot } | { kind: 'game'; session: PlayerSession; snapshot?: RoomSnapshot };
...
export default function App() {
  ...
  const [privateState, setPrivateState] = useState<import('./api/types').PlayerPrivateGameState>();
  ...
  const restore = useCallback(async (session: PlayerSession): Promise<void> => {
    try {
      ...
      setPrivateState(resumed.privateState);
  ...
  useEffect(() => gameHubConnection.onPrivateState((candidate) => {
    setPrivateState((current) => current && current.questionInstanceId === candidate.questionInstanceId ? {
      ...candidate,
  ...
  async function submit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      savePlayerSession(session); setPrivateState(joined.privateState); setScreen({ kind: 'lobby', session, snapshot: joined.snapshot }); void attach(session);
