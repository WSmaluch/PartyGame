...
Media są obsługiwane przez neutralne `IMediaStorage`; lokalny provider zapisuje wyłącznie opaque klucze pod trwałym, konfigurowalnym rootem aplikacji. `MediaAsset` zawiera typ (`ProfilePhoto`, `PhotoAnswer`, `DrawingAnswer`), pokój, gracza i opcjonalne pytanie oraz metadane wariantów. Profile wskazują aktualny asset przez `Player.ProfilePhotoMediaAssetId`; odpowiedzi wskazują asset przez submission. Endpointy API wyszukują metadane przed strumieniowaniem pliku, więc kontrakty klientów nie ujawniają ścieżek systemu plików.

Diagnostyka lokalnego storage jest osobną granicą `IMediaStorageDiagnosticsService`, a nie rozszerzeniem ogólnego kontraktu uploadu. `GET /health/storage` i health check `media-storage` wykonują na żądanie cache’owany probe write/read/delete pod `.diagnostics`, odczytują metryki wolumenu oraz liczą `MediaAsset` i wyłącznie rozpoznane finalne pliki katalogu. Statusy `Healthy`, `Degraded` i `Unhealthy` wynikają z probe oraz konfigurowanych progów wolnego miejsca; odpowiedź i logi nie ujawniają fizycznych ścieżek, mount pointów ani storage keys. Backup wolumenu, alerting, retencja i provider chmurowy należą do deploymentu/operations.

Media PhotoAnswer są zatwierdzane dwuetapowo: storage zapisuje znormalizowane warianty, a baza wiąże je z `MediaAsset` i submission w transakcji. Błąd commit uruchamia kompensacyjne usunięcie obu wariantów. Provider kontroluje limit, magic bytes i format odczytany przez ImageSharp, zapisuje przez `.tmp` i atomowe przeniesienie oraz usuwa metadane obrazu. Restart zachowuje SQLite i katalog mediów; brak fizycznego pliku jest stanem kontrolowanym i zwraca 404 bez destabilizacji pokoju. Wyścigi upload/vote/timeout/display są serializowane istniejącą blokadą pokoju, a indeksy unikalne stanowią drugą linię ochrony.
