...
ConnectionStrings__PartyGame="Data Source=$RUN_DIR/categories.db" ASPNETCORE_ENVIRONMENT=Development ASPNETCORE_URLS="http://127.0.0.1:$API_PORT" dotnet run --no-launch-profile --project "$ROOT_DIR/server/PartyGame.Api" >"$RUN_DIR/api.log" 2>&1 & API_PID=$!
