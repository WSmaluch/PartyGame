...
pre_output="$("$SCRIPT_DIR/backup-data.sh" --deploy-root "$DEPLOY_ROOT" --backup-root "$BACKUP_ROOT" --maintenance --name "$(date -u +%Y%m%dT%H%M%SZ)-$$-pre-restore")"; pre_backup="${pre_output#BACKUP_PATH=}"
...
if mv "$INCOMING/database/partygame.db" "$DB" && [[ "${PARTYGAME_RESTORE_TEST_FAIL_AFTER_DATABASE_SWAP:-false}" != true ]] && mv "$INCOMING/media" "$MEDIA" && data_sqlite_integrity "$DB"; then
  rm -f "$old_db"; rm -rf "$old_media"; rmdir "$INCOMING" 2>/dev/null || true; INCOMING=""; echo "RESTORE_PASS preRestoreBackup=$pre_backup"; exit 0
fi
